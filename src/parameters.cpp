...
int LM_DAMING_TYPE; //阻尼策略
int DL_SCALE_TYPE; //阻尼策略
int OPT_METHOD; //优化方法
int PLOT_CHI_LAMBDA;
...
void Viz_readParameters(string config_file)
{
    // string config_file;
    ...
    // {
    //     cerr << "ERROR: Wrong loop_path to settings" << endl;
    // }
    cv::FileStorage fsSettings(config_file, cv::FileStorage::READ);
    if (!fsSettings.isOpened())
    {
        cerr << "1 readParameters ERROR: Wrong loop_path to settings!" << endl;
        return;
    }
    ...
    // cout << "result loop_path " << VINS_RESULT_PATH << endl;
    ...
    LM_DAMING_TYPE = fsSettings["daming_type"];
    DL_SCALE_TYPE = fsSettings["scale_type"];
    OPT_METHOD = fsSettings["opt_method"];
    TEST_PARAM = fsSettings["test_param"];
    MULTI_THREAD = fsSettings["multi_thread"];
    ...
    fsSettings["data_type"] >> DATA_TYPE;
